...
namespace Lightning
{
	namespace Render
	{
		...
		//A committed draw command.The command is read-only,used for committed draw command.
		struct ICommittedDrawCommand
		{
			virtual ~ICommittedDrawCommand() = default;
		...
		struct IDrawCommand : ICommittedDrawCommand
		{
			...
			virtual ICommittedDrawCommand* Commit()const = 0;
		};

		//DrawCommandQueue is a very important concept.Every drawable object will encode there data to a DrawCommand struct.
		...
		//render resource validation during this frame(render resources won't be deleted because a draw command
		//holds all shared_ptr referring to them).
		using DrawCommandQueue = std::vector<ICommittedDrawCommand*>;
	}
}
