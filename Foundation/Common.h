...
namespace Lightning
{
	...
	{
		//Trick compiler to think of it as a template class
		template<typename T>
		class UtilityImpl
		{
			static_assert(std::is_same<T, std::uint32_t>::value, "T must be std::uint32");
		public:
			static T Hash(const void* key, std::size_t len, T seed)
			{
				T out;
			...
			static void Hash(const void* key, std::size_t len, std::uint32_t seed, void* out)
			{
